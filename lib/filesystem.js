...
function getDevicesWin(diskDrives) {
  ...
  diskDrives.forEach(element => {
    ...
    const mediaType = util.getValue(lines, 'mediaType', ':');
    let partitions = element.split('@{DeviceID=');
    if (partitions.length > 1) {
      partitions = partitions.slice(1);
      partitions.forEach(partition => {
        result.push({ name: partition.split(';')[0].toUpperCase(), device, mediaType });
      });
    }
  });
  return result;
...
function matchDevicesWin(data, diskDrives) {
  ...
  data.map(element => {
    ...
    if (filteresDevices.length > 0) {
      ...
      element.removable = filteresDevices[0].mediaType  === 'External hard disk media' || filteresDevices[0].mediaType === 'Removable Media';
      filteresDevices[0].mediaType  === 'External hard disk media' || filteresDevices[0].mediaType === 'Removable Media' ? element.physical = 'Removable' : '' ;
    }
    return element;
  });
  return data;
    ...
    process.nextTick(() => {
      ...
      if (_windows) {
        ...
        try {
          ...
          const workload = [];
          workload.push(util.powerShell('Get-CimInstance -ClassName Win32_volume | select Caption,DriveType,Name,FileSystem,Size,Capacity,FreeSpace,SerialNumber, Label, Automount, DirtyBitSet | fl'));
          workload.push(util.powerShell('Get-WmiObject -Class Win32_diskdrive | Select-Object -Property PNPDeviceId,DeviceID, Model, Size,MediaType, @{L=\'Partitions\'; E={$_.GetRelated(\'Win32_DiskPartition\').GetRelated(\'Win32_logicaldisk\') | Select-Object -Property DeviceID, VolumeName, Size, FreeSpace}} | fl'));
          util.promiseAll(
            workload
          ).then((res) => {
            ...
            logicalDisks.forEach(function (device) {
              ...
              if (drivetype || true) {
                data.push({
                  ...
                  mount: util.getValue(lines, 'automount', ':'),
                  size: util.getValue(lines, 'capacity', ':'),
                  physical: util.getValue(lines, 'dirtybitset', ':') === "False" ? "Removable" : (drivetype >= 0 && drivetype <= 6) ? drivetypes[drivetype] : drivetypes[0],
                  uuid: util.getValue(lines, 'SerialNumber', ':'),
                  label: util.getValue(lines, 'label', ':'),
                  model: '',
                  serial: util.getValue(lines, 'SerialNumber', ':'),
                  removable: util.getValue(lines, 'dirtybitset', ':') === "False" ? true : drivetype === '2',
                  protocol: '',
                  group: '',
                  device: ''
                });
